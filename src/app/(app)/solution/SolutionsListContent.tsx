...
export default function SolutionsListContent() {
  ...
  const hasActiveFilters =
    searchTerm || selectedTag || statusFilter !== 'all' || difficultyFilter !== 'all' || sortBy !== 'newest'
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm lg:text-base text-gray-600 dark:text-gray-300">
            Browse and manage your problem-solution documentation
          </p>
        </div>
        <Link
        ...
        {filteredSolutions.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredSolutions.map((solution) => (
              <div
                key={solution.id}
                className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-700 px-4 lg:px-6 py-4 lg:py-6 rounded-lg shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex justify-between items-start mb-4">
                  <h2 className="text-lg lg:text-xl font-semibold text-gray-900 dark:text-white pr-4">
                    {solution.title}
                  </h2>
