...
export default function CodeSnippets() {
  const { control } = useFormContext<SolutionForm>()
      ...
      {fields.map((field, index) => (
        <div key={field.id} className="border border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-6 mb-3">
          <div className="flex gap-2 items-center mb-4">
            <input
              {...control.register(`codeSnippets.${index}.title`)}
            ...
            <select
              {...control.register(`codeSnippets.${index}.language`)}
            ...
            <button type="button" className="hover:bg-slate-100 dark:hover:bg-slate-500 p-2 rounded-lg" onClick={() => remove(index)}>
              <X />
            </button>
          </div>

          <textarea
            {...control.register(`codeSnippets.${index}.code`)}
