...
export default function TagsContent({
  ...
  const tagStats = solutions.reduce<Record<string, TagStat>>((acc, solution) => {
    solution.tags.forEach((tag) => {
      if (!acc[tag]) {
        acc[tag] = { name: tag, count: 0, lastUsed: solution.createdAt }
      }
      acc[tag].count += 1
      if (new Date(solution.createdAt).getTime() > new Date(acc[tag].lastUsed).getTime()) {
        acc[tag].lastUsed = solution.createdAt
      }
    })
    return acc
  }, {})
  ...
  const averageTagsPerSolution =
    totalSolutions > 0
      ? (solutions.reduce((sum, sol) => sum + sol.tags.length, 0) / totalSolutions).toFixed(1)
      : 0

  return (
    <div className="p-4 lg:p-6 bg-gray-50 dark:bg-slate-800 min-h-full">
      <div className="mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-2">Tags & Categories</h1>
          ...
          {tagList.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {tagList.map((tag) => (
                <Link
                  key={tag.name}
                  href={`/solution?tag=${encodeURIComponent(tag.name)}`}
