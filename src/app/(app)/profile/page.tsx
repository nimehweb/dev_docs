...
export default function ProfilePage() {
  ...
  const favoritesCount = placeholderFavorites.length
  const successRate = totalSolutions > 0 ? Math.round((resolvedSolutions / totalSolutions) * 100) : 0

  const recentSolutions = placeholderSolutions
    .slice()
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
    .slice(0, 5)

  const tagCounts = placeholderSolutions.reduce<Record<string, number>>((acc, solution) => {
